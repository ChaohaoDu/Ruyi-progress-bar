...
public class ProgressBarUi extends BasicProgressBarUI {
    ...
    protected void paintIndeterminate(Graphics graphics, JComponent component) {
        ...
        var barInsets = progressBar.getInsets();
        ...
        if (component.isOpaque()) {
            graphics2D.fillRect(0, (component.getHeight() - progressBarHeight) / 2, progressBarWidth, progressBarHeight);
        }
        ...
        final var containingRoundRect = new Area(new RoundRectangle2D.Float(1f, 1f, progressBarWidth - 2f, progressBarHeight - 2f, R, R));
        ...
        if (offset2 <= 2) {
            offset2 = 2;
            velocity = 1;
        } else if (offset2 >= progressBarWidth - JBUI.scale(15)) {
            graphics2D.rotate(Math.toRadians(90));
            offset2 = progressBarWidth - JBUI.scale(15);
            velocity = -1;
        }

        var area = new Area(new Rectangle2D.Float(0, 0, progressBarWidth, progressBarHeight));
        area.subtract(new Area(new RoundRectangle2D.Float(1f, 1f, progressBarWidth - 2f, progressBarHeight - 2f, R, R)));
        if (component.isOpaque()) {
            graphics2D.fill(area);
        }
        area.subtract(new Area(new RoundRectangle2D.Float(0, 0, progressBarWidth, progressBarHeight, R2, R2)));

        if (component.isOpaque()) {
            graphics2D.fill(area);
        }

        var state = ProgressBarUiState.getInstance();
        var currentIcon = Icon.loadIcon(cleanURL(state.getCurrentIconPath()));

        currentIcon.paintIcon(progressBar, graphics2D, offset2 - JBUIScale.scale(5), -(component.getHeight() - barRectHeight) / 2);

        graphics2D.draw(new RoundRectangle2D.Float(1f, 1f, progressBarWidth - 2f - 1f, progressBarHeight - 2f - 1f, R, R));
        graphics2D.translate(0, -(component.getHeight() - progressBarHeight) / 2);
    ...
    protected void paintDeterminate(Graphics graphics, JComponent component) {
        ...
        var state = ProgressBarUiState.getInstance();
        var currentBackground = Backgrounds.loadBackground(this.getClass().getResource(state.getCurrentBackgroundPath()));
